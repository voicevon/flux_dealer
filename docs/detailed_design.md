...
| **使能屏蔽位** | `EN_PIN_0` 至 `EN_PIN_7` | `GPIO 13, 12, 14, 27, 26, 25, 33, 32` | 输出 (Output) | 分别控制 M0 至 M7 电机的 A4988 使能引脚 (低电平激活，高电平屏蔽) |
...
*   **多路 GPIO 使能屏蔽控制**：
    A4988 驱动芯片的 `ENABLE` 引脚为低电平使能。
    *   **运动放行 (Bit = 1)**：写入 `LOW`。对应电机通电，并忠实消费 `SHARED_STEP_PIN` 上的每一次脉冲，实现选择性旋转。
    *   **运动屏蔽 (Bit = 0)**：写入 `HIGH`。对应电机驱动器芯片关闭脉冲响应，但系统依然通过芯片拉偏或自锁机制维持锁步力矩（Holding Torque），防止分拣机构因为芦笋的物理撞击或自重产生滑动错位。
